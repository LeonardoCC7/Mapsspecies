...
enf_sp1 = ensemble(m_sp1, 
                   biof_crop,
                   filename='enf_sp1.img',
                   setting=list(method='weighted',
                                stat='tss',
                                opt=2),
                   overwrite=T)
plot(enf_sp1)
enf_sp2 = ensemble(m_sp2,
                   biof_crop,
                   filename='enf_sp2.img',
                   setting=list(method='weighted',
                                stat='tss',
                                opt=2),
                   overwrite=T)
...
plot(en_sp1, col=cl(200), main="Current")
plot(enf_sp1, col=cl(200), main="Projected (RCP 2.6, 50 years)")

par(mfrow=c(1,2)) # more plots in the same frame (1 row, 2 columns)
plot(en_sp2, col=cl(200), main="Current")
plot(enf_sp2, col=cl(200), main="Projected (RCP 2.6, 50 years)")

proj4string(sp1) = projection(en_sp1)
proj4string(sp2) = projection(en_sp2)

p_en_sp1 = en_sp1
p_en_sp2 = en_sp2
p_en_sp1[] = ifelse(en_sp1[] >= 0.5, 1, 0)
p_en_sp2[] = ifelse(en_sp2[] >= 0.5, 1, 0)

p_enf_sp1 = enf_sp1
p_enf_sp2 = enf_sp2
p_enf_sp1[] = ifelse(enf_sp1[] >= 0.5, 1, 0)
p_enf_sp2[] = ifelse(enf_sp2[] >= 0.5, 1, 0)

# Continuous 
mapview(en_sp1,col.regions=cl(200)) +
  mapview(enf_sp1,col.regions=cl(200)) +
  sp1
mapview(en_sp2,col.regions=cl(200)) +
  mapview(enf_sp2,col.regions=cl(200)) +
  sp2

ch_sp1 = enf_sp1 - en_sp1
cl2 = colorRampPalette(c('red','orange','yellow','gray','green','blue'))
plot(ch_sp1,col=cl2(200),main="Habitat suitability variation sp1")
ch_sp2 = enf_sp2 - en_sp2
cl2 = colorRampPalette(c('red','orange','yellow','gray','green','blue'))
...
pa_sp1 = raster(en_sp1)
pa_sp1[] = ifelse(en_sp1[] >= 0.5887408, 1, 0)
cl = colorRampPalette((c("red","grey80","blue")))
plot(pa_sp1,col=cl(3),main="Predicted Current presences sp1")
paf_sp1 = raster(enf_sp1)
paf_sp1[] = ifelse(enf_sp1[] >= 0.5887408, 1, 0)
cl = colorRampPalette((c("red","grey80","blue")))
...
pa_sp2 = raster(en_sp2)
pa_sp2[] = ifelse(en_sp2[] >= 0.7343112, 1, 0)
cl = colorRampPalette((c("red","grey80","blue")))
plot(pa_sp2,col=cl(3),main="Predicted Current presences sp2")
paf_sp2 = raster(enf_sp2)
paf_sp2[] = ifelse(enf_sp2[] >= 0.7343112, 1, 0)
cl = colorRampPalette((c("red","grey80","blue")))
...
par(mfrow=c(1,2)) # more plots in the same frame (1 row, 2 columns)
plot(en_sp1, col=cl(200), main="Current")
plot(enf_sp1, col=cl(200), main="Projected (RCP 8.5, 70 years)")

par(mfrow=c(1,2)) # more plots in the same frame (1 row, 2 columns)
plot(en_sp2, col=cl(200), main="Current")
plot(enf_sp2, col=cl(200), main="Projected (RCP 8.5, 70 years)")
